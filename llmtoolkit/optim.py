    ...
    def step(self, step_idx: int = 0):
        ...
        for param_group, base_lr in zip(self.optimizer.param_groups, self.base_lrs):
            if step_idx < warmup_length:
                ...
                lr = base_lr * max(
                    0.0,
                    float(self.max_steps - step_idx)
                    / float(max(1, self.max_steps - warmup_length)),
                )
        ...
        for group in self.param_groups:
            ...
            for n, p in param_dict.items():
                ...
                if len(state) == 0:
                    ...
                    state["exp_avg"] = (
                        torch.zeros(size).to(p.device).to(p.dtype).to(torch.bfloat16)
                    )
                ...
                if len(param_list) == 2:
                    ...
                    B_TB_inv = torch.linalg.pinv(
                        B_TB + delta * torch.eye(A.shape[0]).to(A.device)
                    )

                    grad_A = (1 / scaling_factor**2) * B_TB_inv @ grad_A_orin
                    grad_B = (1 / scaling_factor**2) * (
                        (torch.eye(B.shape[0]).to(B.device) - B @ B_TB_inv @ B.T)
                        @ grad_B_orin
                        @ AA_T_inv
                    )
                    equiv_grad = (
                        scaling_factor * B @ grad_A + scaling_factor * grad_B @ A
                    )
                ...
                if group["amsgrad"]:
                    ...
                    torch.maximum(
                        state["max_exp_avg_sq"], exp_avg_sq, out=state["max_exp_avg_sq"]
                    )

                    # Use the max. for normalizing running avg. of gradient
                    denom = (
                        state["max_exp_avg_sq"].sqrt() / bias_correction2_sqrt
                    ).add_(group["eps"])
                else:
                ...
                if len(param_list) != 2:
                    ...
                    if self.X_mode == "sylvester":
                        X = solve_sylvester(
                            B.T @ B,
                            A @ A.T,
                            -(1 / scaling_factor**2) * B_TB_inv @ grad_A_orin @ A.T,
                        )
                    ...
                    if group["weight_decay"] != 0:
                        ...
                        param_dict[weight_name].mul_(
                            1 - group["weight_decay"] * group["lr"]
                        )
            ...
            for p, n in zip(group["params"], group["names"]):
                ...
                if len(state) == 0:
                    ...
                    if len(param_list) == 2:
                        ...
                        # Exponential moving average of gradient values
                        state["exp_avg_A"] = (
                            torch.zeros(param_list[0].shape).to(p.device).to(p.dtype)
                        )
                        state["exp_avg_B"] = (
                        ...
                        if group["amsgrad"]:
                            # Maintains max of all exp. moving avg. of sq. grad. values
                            state["max_exp_avg_sq_A"] = (
                                torch.zeros(param_list[0].shape)
                                .to(p.device)
                                .to(p.dtype)
                            )
                ...
                if len(param_list) == 2:
                    ...
                    exp_avg_sq_A.mul_(beta2).addcmul_(
                        grad_A, grad_A.conj(), value=1 - beta2
                    )
                    ...
                    if group["amsgrad"]:
                        ...
                        # Use the max. for normalizing running avg. of gradient
                        denom_A = (
                            state["max_exp_avg_sq_A"].sqrt() / bias_correction2_sqrt
                        ).add_(group["eps"])
                        denom_B = (
                            state["max_exp_avg_sq_B"].sqrt() / bias_correction2_sqrt
                        ).add_(group["eps"])
                    else:
                        denom_A = (exp_avg_sq_A.sqrt() / bias_correction2_sqrt).add_(
                            group["eps"]
                        )
