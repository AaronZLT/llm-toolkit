...
class BenchTracer(object):
    ...
    def extract_stream(self):
        ...
        for meta_data in self.raw_data["traceEvents"]:
            ...
            if stream == 7:
                ...
                stream84_kernel.append(meta_data)
        ...
        self.print_info(
            "stream kerenl num",
            [f"{stream.name} {len(stream.kernel_list)}" for stream in self.stream_list],
        )
    ...
    def extract_stage(self):
        ...
        forward_stage = BenchStage(
            "forward",
            forward_start_kernel_list,
            forward_end_kernel_list,
            self.stream_list[0],
            self.config,
        )
        backward_stage = BenchStage(
            "backward",
            backward_start_kernel_list,
            backward_end_kernel_list,
            self.stream_list[0],
            self.config,
        )
        optimize_stage = BenchStage(
            "optimize",
            optimize_start_kernel_list,
            optimize_end_kernel_list,
            self.stream_list[0],
            self.config,
        )
