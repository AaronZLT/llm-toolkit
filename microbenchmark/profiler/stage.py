...
class BenchStage(object):
    def __init__(self, name, start_kernel_list, end_kernel_list, stream, config):
        self.name = name
    ...
    def match(self):
        def check_match(idx, kernel_list):
            ...
            for incre_ii in range(len(kernel_list)):
                if (
                    kernel_list[incre_ii]
                    not in self.stream.kernel_list[ii + incre_ii]["name"]
                ):
                    return False, 0
        ...
        for ii in range(len(self.stream.kernel_list)):
            ...
            if flag == True:
                self.end_time.append(
                    self.stream.kernel_list[ii + skip]["ts"]
                    + self.stream.kernel_list[ii + skip]["dur"]
                )
    ...
    def extract_fine_module(self, module_names):
        ...
        for module_name in module_names:
            fine_module_kernel_dict[module_name] = self.config["fine_module"][
                module_name
            ]
        ...
        def check_match_find_module(idx, name, kernel_name_list):
            ...
            for kernel_ii in range(len(kernel_name_list)):
                if (
                    kernel_name_list[kernel_ii]
                    not in self.stream.kernel_list[idx + kernel_ii]["name"]
                ):
                    return False, 0, None
                kernel_list.append(
                    BenchKernel(
                        kernel_name_list[kernel_ii],
                        self.stream.kernel_list[idx + kernel_ii]["ts"],
                        self.stream.kernel_list[idx + kernel_ii]["dur"],
                    )
                )
            return (
                True,
                len(kernel_name_list) - 1,
                BenchModule(name, kernel_list, self.stream),
            )
        ...
        for idx in range(len(self.stream.kernel_list)):
            if skip != 0:
                ...
                continue
            for module_name in module_names:
                flag, skip, module = check_match_find_module(
                    idx, module_name, fine_module_kernel_dict[module_name]
                )
                if flag == True:
        ...
        self.print_info(
            f"{self.name} fine_module time",
            [
                f"{name}: {sum_fine_modules_dur_time[name]}, num: {num_fine_modules_dur_time[name]}, avg: {sum_fine_modules_dur_time[name]/num_fine_modules_dur_time[name]}"
    ...
    def extract_coarse_module(self):
        ...
        for coarse_module_name in coarse_module_names:
            coarse_module_contains_module_dict[coarse_module_name] = self.config[
                "coarse_module"
            ][coarse_module_name]
